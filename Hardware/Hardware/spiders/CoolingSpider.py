...
from Hardware.items import CoolingItem
from Hardware.loaders import CoolingLoader
import json

class CoolingSpider(scrapy.Spider):
    name = "CoolingSpider"
    ...
    def load_product_links(self):
        try:
            with open("C:\\Users\\jacob\\OneDrive - Aston University\\Programming\\Python\\HardwareSpiders\\Hardware\\fruits\\newlinks.json", "r") as file:
                ...
                return [item["link"] for item in data if item.get("category") == "CPU Fan"]
    ...
    def start_requests(self):
        cooling_links = self.load_product_links()

        for url in cooling_links:
            self.driver.get(url)
    ...
    def parse(self, response):
        ...
        loader = CoolingLoader(item=CoolingItem(), selector=selector)
        ...
        for row in table_rows:
            if row.css("th::text").get():
                ...
                if row_title == "Compatible Sockets":
                    sockets = row_value.split(", ")
                    loader.add_value("compatible_sockets", sockets)
