...
export default function Sidebar({subtotal, delivery_cost, vat, total}){
  return (
    <Stack className="px-8 rounded-md py-10 w-[350px]">
      ...
      <h3 decimal={2}>Delivery cost: {Intl.NumberFormat('en-GB', { maximumSignificantDigits: 2, currency: 'GBP', style: "currency" }).format(delivery_cost)}</h3>
      <h3 decimal={2}>VAT: {Intl.NumberFormat('en-GB', { maximumSignificantDigits: 2, currency: 'GBP', style: "currency" }).format(vat)}</h3>
      <hr/>
      <h3>Total: {total}</h3>
