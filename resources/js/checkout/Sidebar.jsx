  ...
  return (
    <Stack>
      ...
      <Flex align="baseline" gap="15">
        <Title order={6}>Delivery cost: {Intl.NumberFormat('en-GB', { maximumSignificantDigits: 6, currency: 'GBP', style: "currency" }).format(delivery_cost)}</Title>
        <Title order={6}>VAT: {Intl.NumberFormat('en-GB', { maximumSignificantDigits: 2, currency: 'GBP', style: "currency" }).format(vat)} </Title>
  ...
  return (
    <Flex gap="20">
      ...
      <Stack>
        ...
        <h3>Quantity: {qty}</h3>
        <h2>Unit Price: {Intl.NumberFormat('en-GB', { maximumSignificantDigits: 2, currency: 'GBP', style: "currency" }).format(qty)}</h2>
        <h2>Total Price: {Intl.NumberFormat('en-gb', { maximumsignificantdigits: 2, currency: 'gbp', style: "currency" }).format(price * qty)}</h2>
      </Stack>
    </Flex>
  )
}
